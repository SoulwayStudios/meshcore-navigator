...
class PixooRenderer:
    """Renders tabbed channel pages with message queueing, 10s vibrant hold, and dynamic backlog pacing."""
    ...
    def __init__(self, config=None):
        ...
        # Message Display FIFO Queue & Dynamic Backlog Acceleration Engine
        self.message_queue: List[Tuple[str, ChannelMessage]] = []
        self.active_message: Optional[ChannelMessage] = None
        self.active_channel_name: Optional[str] = None
        self.message_display_start: float = time.time()
    ...
    def get_effective_display_duration(self) -> float:
        """Calculates display time based on backlog.
        
        If cumulative queue display time at 10s exceeds 60s (more than 6 queued messages),
        duration is reduced to 3s per message until queue backlog is under 1 minute.
        """
        if len(self.message_queue) * 10.0 > 60.0:
            return 3.0
        return 10.0

    def trigger_message_alert(self, message: MessageEnvelope):
        """Processes an incoming message, queues it or presents it immediately."""
        ...
        now = time.time()
        eff_dur = self.get_effective_display_duration()
        eff_frames = int(eff_dur * 25)

        # Check if currently busy displaying an active message within its vibrant hold window
        is_busy = (self.active_message is not None and (now - self.message_display_start < eff_dur) and self.anim_frame < eff_frames)

        # Support pending channel switch tracking for tests
        ...
        if not is_busy and not self.message_queue:
            self._display_message(clean_chan, msg_entry)
        else:
            self.message_queue.append((clean_chan, msg_entry))

        # Return from Telemetry or Neighbours mode if active
        if self.mode in (DisplayMode.TELEMETRY, DisplayMode.NEIGHBOURS):
            self.mode = DisplayMode.CAROUSEL

    def _display_message(self, channel_name: str, msg: ChannelMessage):
        """Switches active message and channel, resetting animation ticks and flash alert."""
        self.active_message = msg
        self.active_channel_name = channel_name
        self.latest_messages[channel_name] = msg
        now = time.time()
        self.message_display_start = now
        self.page_start_time = now
        self.last_switch_time = now
        self.anim_frame = 0

        self._switch_to_channel_page(channel_name, now)
    ...
    def render_frame(self) -> Image.Image:
        """Main rendering pipeline with message queue pacing, slide transitions, and inverted header alert."""
        now = time.time()
        self.anim_frame += 1

        # Check queued message progression: switch to next message after at least 10s (or 3s accelerated)
        if self.message_queue:
            eff_dur = self.get_effective_display_duration()
            eff_frames = int(eff_dur * 25)
            if (now - self.message_display_start) >= eff_dur or self.anim_frame >= eff_frames:
                next_chan, next_msg = self.message_queue.pop(0)
                self._display_message(next_chan, next_msg)
        elif self.active_message is None and self.latest_messages:
            active_pages = self.get_active_channel_pages()
            curr = active_pages[self.current_page_idx % len(active_pages)].lower()
            if curr in self.latest_messages and self.latest_messages[curr]:
                self.active_message = self.latest_messages[curr]
                self.active_channel_name = curr

        # Check legacy pending channel switch
    ...
    def _render_carousel(self, img: Image.Image, draw: ImageDraw.Draw, now: float, is_header_inverted: bool):
        ...
        # Timed carousel rotation (only when no queued messages are waiting to display)
        if not self.message_queue and not self.pending_channel_switch and not self.is_transitioning and len(active_pages) > 1 and elapsed_page >= page_dur:
        ...
        if self.is_transitioning:
            ...
            if trans_elapsed >= self.transition_duration:
                ...
                x_offset = int(progress * 64)
                prev_page_name = active_pages[self.prev_page_idx % len(active_pages)]
                curr_page_name = active_pages[self.current_page_idx % len(active_pages)]
                prev_img = self._render_channel_page(prev_page_name, False)
                curr_img = self._render_channel_page(curr_page_name, is_header_inverted)
                img.paste(prev_img, (-x_offset, 0))
                img.paste(curr_img, (64 - x_offset, 0))
        else:
            page_name = active_pages[self.current_page_idx % len(active_pages)]
            page_img = self._render_channel_page(page_name, is_header_inverted)
            img.paste(page_img, (0, 0))
    ...
    def _render_channel_page(self, channel_name: str, is_header_inverted: bool) -> Image.Image:
        """Renders single latest message with vibrant hold, queue protection, and fade-to-dull-grey."""
        page = Image.new("RGB", (64, 64), (0, 0, 0))
        p_draw = ImageDraw.Draw(page)

        # 1. Top Channel Heading (y=0..9)
        chan_col = self.get_channel_color(channel_name)
        divider_col = self._blend_color(chan_col, (40, 45, 55), 0.5)
        ...
        is_chan_fav = bool(self.config and self.config.is_channel_favorite(channel_name))
        header_title = f"#{channel_name.upper()}"
        if is_header_inverted:
            p_draw.rectangle([0, 0, 63, 8], fill=chan_col)
            star_prefix = "★ " if is_chan_fav else ""
            self._draw_text(page, f"{star_prefix}{header_title}"[:12], 2, 1, (0, 0, 0))
        else:
            if is_chan_fav:
                self._draw_glyph(page, GLYPH_STAR, 2, 1, (255, 68, 68))
                self._draw_text(page, header_title[:10], 8, 1, chan_col)
            else:
        ...
        msg = None
        if self.active_message and self.active_channel_name and self.active_channel_name.lower() == clean_name.lower():
        ...
        # 3. Timing, Bounce & Fade-to-Dull-Grey Logic
        # Rules:
        # - Short Message (<= 44px): display in full color for 10 seconds (or 3s accelerated),
        #   then if no queued messages wait, smoothly fade to dull grey over 1.5s (38 frames).
        # - Long Message (> 44px): executes top hold (5s) -> scroll down -> bottom hold (4s) -> scroll up.
        #   Once back at top, hold for 10s (or 3s), then fade to dull grey over 1.5s (38 frames).
        bounce_y = 0.0
        dim_factor = 0.0

        FADE_DURATION_FRAMES = 38  # ~1.5 seconds smooth fade

        # Pre-calculate message height for timing
        temp_lines = self._wrap_and_tokenize(msg.text, base_color=COLOR_BODY_WHITE, max_chars_per_line=12)
        total_msg_height = len(temp_lines) * 7
        available_h = 44

        eff_dur = self.get_effective_display_duration()
        HOLD_SHORT_FRAMES = int(eff_dur * 25)
        ...
        if total_msg_height <= available_h:
            ...
            if self.anim_frame < HOLD_SHORT_FRAMES:
                ...
                # If there are queued messages, stay 100% vibrant until next message switches
                dim_factor = 0.0
            elif self.anim_frame < HOLD_SHORT_FRAMES + FADE_DURATION_FRAMES:
                dim_factor = (self.anim_frame - HOLD_SHORT_FRAMES) / float(FADE_DURATION_FRAMES)
            else:
                dim_factor = 1.0
        else:
            # Long Message
            ...
            HOLD_AFTER_SCROLL_FRAMES = int(eff_dur * 25)
            ...
            if self.anim_frame < pause_top_frames:
                ...
                # 5. HOLD at top for 10 seconds (or 3s accelerated) after scroll completes
                bounce_y = 0.0
                dim_factor = 0.0
            elif self.message_queue:
                # If queued messages exist, ignore fade completely
                bounce_y = 0.0
                dim_factor = 0.0
            elif self.anim_frame < fade_start_frame + FADE_DURATION_FRAMES:
                # 6. Fade to dull grey over 1.5s
                bounce_y = 0.0
                dim_factor = (self.anim_frame - fade_start_frame) / float(FADE_DURATION_FRAMES)
            else:
                # 7. Rest permanently at top in dull grey
                bounce_y = 0.0
                dim_factor = 1.0

        # Dull grey color definitions
        DULL_GREY_TEXT = (80, 85, 90)
        DULL_GREY_SENDER = (70, 75, 80)
        DULL_GREY_TIME = (50, 55, 60)
        DULL_GREY_STAR = (80, 80, 85)

        eff_sender_col = self._blend_color(sender_col, DULL_GREY_SENDER, dim_factor)
        eff_star_col = self._blend_color(star_col, DULL_GREY_STAR, dim_factor)
        eff_time_col = self._blend_color(time_col, DULL_GREY_TIME, dim_factor)
        eff_body_col = self._blend_color(COLOR_BODY_WHITE, DULL_GREY_TEXT, dim_factor)
        eff_tag_col = self._blend_color(COLOR_TAG_BLUE, DULL_GREY_TEXT, dim_factor)

        # 4. User Header Line (y=11..17): [★] @username time
        ...
        # Horizontal pan if user header overflows 60px (only during active full color)
        x_shift = 0
        if header_width > 60 and dim_factor < 1.0:
        ...
        x_pos = 1 - x_shift
        if msg.is_favorite:
            self._draw_glyph(header_band, GLYPH_STAR, x_pos, 0, eff_star_col)
            x_pos += 6

        user_tag = f"@{msg.sender_name}"
        self._draw_text(header_band, user_tag, x_pos, 0, eff_sender_col)
        x_pos += (len(user_tag) + 1) * 5

        self._draw_text(header_band, msg.time_str, x_pos, 0, eff_time_col)
        cropped_header = header_band.crop((0, 0, 64, 7))

        # 5. Tokenize and Wrap Single Latest Message Body
        line_tokens = self._wrap_and_tokenize(
            msg.text,
            base_color=eff_body_col,
            tag_color=eff_tag_col,
    ...
    def _blend_color(self, c1: Tuple[int, int, int], c2: Tuple[int, int, int], factor: float) -> Tuple[int, int, int]:
        """Blends color c1 towards c2 by factor (0.0=c1, 1.0=c2)."""
    ...
    def _wrap_and_tokenize(
        self,
        text: str,
        base_color: Tuple[int, int, int],
        tag_color: Tuple[int, int, int] = COLOR_TAG_BLUE,
        max_chars_per_line: int = 12
    ) -> List[List[Tuple[str, Tuple[int, int, int]]]]:
        """Wraps text into lines, highlighting words starting with '#' or '@' in tag_color."""
        words = text.split()
