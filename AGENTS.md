# AGENTS.md

## Repository Instructions & Versioning Rule

1. **Versioning Rule**:
   - Current base version is `0.0.2`.
   - On **ANY new commit** to GitHub: increment the patch version by `0.0.1` (e.g. `0.0.2` -> `0.0.3`).
   - On **new features**: increment the minor version by `0.1.0` (e.g. `0.0.3` -> `0.1.0`).
   - Files to update: `meshcore_tray/__init__.py` and `pyproject.toml`.
   - You can run `python3 scripts/bump_version.py` (or `--feature`).

2. **Early Development Disclaimer**:
   - Maintain the early development notice and Coffee link (`https://buymeacoffee.com/m7ncy`) in `README.md` and app About/Splash tabs.
